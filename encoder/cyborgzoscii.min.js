...
function toZOSCII(g,a,m,k,r,n){var t=(new Date).getTime(),b,s=0,h=0,p=0;n=Array(256);var e=Array(256),c=Array(256),q=Array(256),d,l,f;for(b=0;256>b;b++)n[b]=0,q[b]=0;for(b=0;b<m.length;b++){f=m[b];for(d=f.start;d<f.start+f.size;d++)l=g[d],n[l]++}for(b=0;256>b;b++)e[b]=Array(n[b]),c[b]=0;for(b=0;b<m.length;b++){f=m[b];for(d=f.start;d<f.start+f.size;d++)l=g[d],e[l][c[l]]=d,c[l]++}for(b=0;b<a.length;b++)c=a.charCodeAt(b),k&&(c=k(c,r)),0<=c&&256>c&&e[c]&&0<e[c].length?h++:(p++,10>=p&&console.log("Missing character: '"+
a.charAt(b)+"' (code "+a.charCodeAt(b)+" -> "+c+")"));console.log("Characters found in ROM: "+h);console.log("Characters missing from ROM: "+p);h=Array(h);for(b=0;b<a.length;b++)c=a.charCodeAt(b),k&&(c=k(c,r)),0<=c&&(256>c&&e[c]&&0<e[c].length)&&(q[c]++,p=Math.floor(Math.random()*e[c].length),h[s]=e[c][p],s++);k=(new Date).getTime()-t;console.log("ZOSCII Performance:");console.log("- Binary size: "+g.length+" bytes");console.log("- Input length: "+a.length+" chars");console.log("- Memory blocks: "+
m.length);console.log("- Execution time: "+k+"ms");console.log("- Output addresses: "+h.length);return{addresses:h,inputCounts:q,romCounts:n}}
