...
function displayAnalysis(objEncodingResult_a) 
{
    ...
    strHtml += "<tr><td><strong>Original Input:</strong></td><td>" + escapeHtml(strDisplayText) + "</td></tr>";

    strHtml += "<tr><td><strong>Input Length:</strong></td><td>" + objEncodingResult_a.inputLength + " bytes</td></tr>";
...
function displayLeakageAnalysis()
{
    ...
    strHtml += "<table class='table table-sm analysis-table'>";
    strHtml += "<colgroup><col style='width: 40%;'><col style='width: 60%;'></colgroup>";
    strHtml += "<tr><td><strong>Total Unique Addresses:</strong></td><td>" + intUniqueAddresses + "</td></tr>";
    strHtml += "<tr><td><strong>Total Leaked Patterns:</strong></td><td>" + g_objLeakageTracker.leakedPatterns.length + "</td></tr>";
    strHtml += "</table>";
...
function displayROMStrength(objEncodingResult_a)
{
	...
	strHtml += "<tr><td><strong>General ROM Capacity:</strong></td><td>~10^" + fltGeneralStrength.toFixed(0) + "</td><td>" + exponentToLayman(fltGeneralStrength) + "</td></tr>";
	strHtml += "<tr><td><strong>This File Security:</strong></td><td>~10^" + fltFileStrength.toFixed(0) + "</td><td>" + exponentToLayman(fltFileStrength) + "</td></tr>";
	strHtml += "<tr><td><strong>Characters Utilized:</strong></td><td>" + intCharactersUsed + " of 256 (" + fltUtilization.toFixed(1) + "%)</td><td></td></tr>";
	strHtml += "</table>";
	strHtml += "</div>";
...
function setupButtons() 
{
	...
	document.getElementById("verifyBtn").addEventListener("click", function() 
	{
        ...
        // Prepare input data - if it's a file payload, prepend "FILE:" prefix
        var varMixedInput;
        var intInputLength;
        var strDisplayText;
		...
		{
			varMixedInput = g_arrBinaryInputData;
			intInputLength = g_arrBinaryInputData.length;
			strDisplayText = "[Binary: " + intInputLength + " bytes]";
		}
		else
		{
			varMixedInput = g_strTextData;
			intInputLength = g_strTextData.length;
			strDisplayText = g_strTextData.length > 100 ? g_strTextData.substring(0, 100) + "..." : g_strTextData;
		}
...
{
    var strServerUrl = document.getElementById("txtServerUrl").value.trim();
    var strQueueGUID = document.getElementById("txtQueueGUID").value.trim();
    ...
    if (!strQueueGUID)
    {
        alert("Please enter a queue GUID");
        return;
    }
    
    // Convert addresses to binary (16-bit little-endian)
    ...
    // Create multipart form data matching ZOSCII MQ format
    var objFormData = new FormData();
    objFormData.append('action', 'publish');
    objFormData.append('q', strQueueGUID);
    objFormData.append('r', intRetentionWeeks.toString());
    
    // Create blob and append as file
    var objBlob = new Blob([arrBinary], {type: 'application/octet-stream'});
    objFormData.append('msg', objBlob, 'message.bin');
    
    // Show status
    var objSubmitBtn = document.getElementById("submitBtn");
    var strOriginalText = objSubmitBtn.textContent;
    objSubmitBtn.textContent = "Submitting...";
    objSubmitBtn.disabled = true;
    
    makeAjaxRequest(strServerUrl, {
        method: 'POST',
        data: objFormData
    }, function(strResponse_a) 
    {
        console.log("Response:", strResponse_a);
        
        // Try to parse response
        var objResponse;
        try
        {
            objResponse = JSON.parse(strResponse_a);
        }
        catch(e)
        {
            objResponse = { error: "Invalid server response", detail: strResponse_a };
        }
        
        if (objResponse.error === "" || objResponse.error === null)
        {
            alert("🎺 Successfully submitted to queue: " + strQueueGUID);
        }
        else
        {
            alert("❌ Failed to submit: " + (objResponse.error || "Unknown error"));
        }
        
        objSubmitBtn.textContent = strOriginalText;
        objSubmitBtn.disabled = false;
    }, function(strError_a) 
    {
        console.error("Upload failed:", strError_a);
        
        if (strError_a.length === 0)
        {
            alert("⚠️ Upload Failed (CORS or network issue). Check the browser console for details.\n\nIf the server does not support CORS, you may need to run this from the same origin or use a proxy.");
        }
        else
        {
