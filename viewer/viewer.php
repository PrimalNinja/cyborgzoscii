...
if (file_exists($strFilename)) 
{
	...
	if (BITTAGE == 16)
	{
		...
		for ($intI = 0; $intI < strlen($strBinaryData); $intI += 2) 
		{
			if ($intI + 1 < strlen($strBinaryData)) 
			{
				$intByte1 = ord($strBinaryData[$intI]);
				$intByte2 = ord($strBinaryData[$intI + 1]);
				
				$intAddress = $intByte1 | ($intByte2 << 8);
